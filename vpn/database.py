...
async def ensure_user_exists(user_id: int, username: str = None, first_name: str = None, last_name: str = None):
    """Гарантирует что пользователь существует в БД перед созданием подписки"""
    pool = await get_pool()
    async with pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO users (user_id, username, first_name, last_name) VALUES ($1, $2, $3, $4) ON CONFLICT (user_id) DO NOTHING",
            user_id, username or f"user_{user_id}", first_name, last_name
        )
